...
class ColorController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($data) {
                    ...
                    $btn = $btn.'<a href="' . route('admin.product.color.edit', $data->id) . '" class="btn-action"><i class="fas fa-pencil-alt"></i></a>';
    ...
    public function productColorStore(ProductColorRequest $request){
        ...
        if ($color) {
            return redirect()->route('admin.product.color')->with('toast_success', __('Successfully Stored !'));
        }
    ...
    public function productColorUpdate(Request $request){
        ...
        if ($color) {
            return redirect()->route('admin.product.color')->with('toast_success', __('Successfully Update !'));
        }
    ...
    public function productColorDelete($id){
        ...
        if ($delete) {
            $delete->delete();
            return redirect()->route('admin.product.color')->with('toast_success', __('Successfully Deleted !'));
        }
        return redirect()->route('admin.product.color')->with('toast_error', __('Does Not Delete!'));
    }
}
