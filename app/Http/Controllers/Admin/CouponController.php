...
class CouponController extends Controller
{
    ...
    public function couponStore(CouponRequest $request){
        if($request->min_expenses < $request->amount) {
            return redirect()->back()->with('toast_error', __('Amount never be exceeded from minimum expense'));
        }
        ...
        if ($coupon) {
            return redirect()->route('admin.coupon')->with('toast_success', __('Successfully Stored !'));
        }
    ...
    public function couponDelete($id){
        ...
        if($order_count != 0) {
            return redirect()->back()->with('toast_error', __('This coupon is used by a user. You can not delete used coupon.'));
        }
        $delete = Coupon::Where('id', $id)->delete();
        if (!empty($delete)) {
            return redirect()->route('admin.coupon')->with('toast_success', __('Successfully Deleted !'));
        }
        return redirect()->route('admin.coupon')->with('toast_error', __('Does Not Delete!'));
    }
    ...
    public function couponUpdate(Request $request){
        ...
        if ($update) {
            return redirect()->route('admin.coupon')->with('toast_success', __('Successfully Update !'));
        }
        return redirect()->back()->with('toast_error', __('Does not insert  !'));
    }
}
