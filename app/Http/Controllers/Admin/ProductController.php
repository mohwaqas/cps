...
class ProductController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($data) {
                    ...
                    $btn = $btn.'<a href="' . route('admin.product.duplicate',$data->id) . '" class="btn-action"><i title="Duplicate" class="fas fa-database"></i></a>';
    ...
    public function productStore(ProductRequest $request){ 
        ...
        $data['is_called'] = $request->is_called;
        ...
        $data['en_additionalinformation'] = $request->en_additionalinformation_old;
    ...
    public function physicalProductAdd($data)
    {
        ...
        if($data['en_brand_name'] === '---Select item---') {
            $data['en_brand_name'] = 8;
        }
        
        if($data['en_category_name'] === '---Select item---') {
            $data['en_category_name'] = 1;
        }


        $product = Product::create([
            'en_Product_Name' => $data['en_product_name'] ?? null,
            'en_Product_Slug' => $data['en_product_slug'] ?? null,
            'Brand_Id' => $data['en_brand_name'] ?? null,
            'Category_Id' => $data['en_category_name'] ?? null,
            'Bronze_Price' => $data['bronze_price'] ?? null,
            'Silver_Price' => $data['silver_price'] ?? null,
            'Gold_Price' => $data['gold_price'] ?? null,
            'Platinum_Price' => $data['platinum_price'] ?? null,
            'Price' => $data['price'] ?? null,
            'Discount' => $discount ?? null,
            'Discount_Price' => $data['discount_price'] ?? null,
            'en_About' => $data['en_about'] ?? null,
         ...
         /*   'fr_Product_Name' => $data['fr_product_name'],
            ...
            'Featured_Product' => $data['feature'] ?? 0,
            'Best_Selling' => $data['best_sale'] ?? 0,
            'On_Sale' => $data['on_sale'] ?? 0,
            'New_Arrival' => $data['on_arrival'] ?? 0,
            'Voucher' => $this->generateRandomString(6) ?? null,
            'Spec_Sheet'=>$data['Spec_Sheet'] ?? null, 
            'Spec_Sheet2'=>$data['Spec_Sheet2'] ?? null, 
            'Spec_Sheet3'=>$data['Spec_Sheet3'] ?? null, 
            'Spec_Sheet4'=>$data['Spec_Sheet4'] ?? null, 
            'is_retail_price' => $data['is_retail_price'] ?? null,
            'showing_home' => $data['showing_home'] ?? null,
             'is_called' => $data['is_called'] ?? null,
        ...
        //print_r( $product); die();
        if(!empty($product)){
            if(isset($data['product_tag']) && count($data['product_tag']) > 0)
            ...
            $colorsid = $data['color'] ?? null;
            $sizeid = $data['size_new'] ?? null;
            if($colorsid) {
            ...
            Location::create([
                ...
                'location4'=>$data['location4'] ?? null,
                'location5'=>$data['location5'] ?? null,
                'location_next1'=>$data['location_next1'] ?? null,
                'location_next2'=>$data['location_next2'] ?? null,
                'location_next3'=>$data['location_next3'] ?? null,
                'location_next4'=>$data['location_next4'] ?? null,
                'location_next5'=>$data['location_next5'] ?? null,
                'location_date1'=>$data['location_date1'] ?? null,
                'location_date2'=>$data['location_date2'] ?? null,
                 'product_id' => $product->id,
            ]);
    ...
    public function productUpdate(Request $request){
        ...
        if (!empty($request->image_five)) {
          ...
          if ($request->spec_sheet) {
            ...
            {
                if($request->spec_sheet_del_1 && $request->spec_sheet_del_1==1){
                     $data['Spec_Sheet'] = '';
                }
                else{
          ...
          if ($request->spec_sheet2) {
            ...
            {
                  if($request->spec_sheet_del_2 && $request->spec_sheet_del_2==1){
                     $data['Spec_Sheet2'] = '';
                }
                else{
          ...
          if ($request->spec_sheet3) {
            ...
            {
                 if($request->spec_sheet_del_3 && $request->spec_sheet_del_3==1){
                     $data['Spec_Sheet3'] = '';
                }
                else{
                     $data['Spec_Sheet3'] = $product->spec_sheet3;              
                }
          ...
          if ($request->spec_sheet4) {
            ...
            else 
            {
                ...
                if($request->spec_sheet_del_4 && $request->spec_sheet_del_4==1){
                     $data['Spec_Sheet4'] = '';
                }
                else{
                     $data['Spec_Sheet4'] = $product->spec_sheet4;              
                }
    ...
    public function physicalProductUpdate($data, $product)
    {
        ...
        $update = $product->update([
            'en_Product_Name' => is_null($data['en_product_name']) ? $product->en_Product_Name : $data['en_product_name'],
              'en_Product_Slug' => is_null($data['en_product_slug']) ? $product->en_Product_Name : $data['en_product_slug'], 
        ...
        if(!empty($update)){
            ...
            $colorsid = isset($data['color']) ? $data['color'] : null;
            //$sizeid = $data['size'];
            if($colorsid) {
    ...
    public function duplicate($id){
        $data = Product::with('category','brand')->where('id',$id)->first();//->toArray();
