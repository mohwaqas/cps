...
class ContactUsController extends Controller
{
    public function contactUs(Request $request){
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->rawColumns(['action', 'FirstName', 'LastName', 'Email','ContactNumber','Message', 'date'])
                ->make(true);
        }
    ...
    public function contactUsDelete($id){
        ...
        if ($delete) {
            $delete->delete();
            return redirect()->route('admin.contact.us.index')->with('toast_success', __('Successfully Deleted !'));
        }
        return redirect()->back()->with('toast_error', __('Does Not Delete!'));
    }
    ...
    public function sendReply(Request $request)
    {
        if(is_null($request->message)) {
            return redirect()->back()->with('toast_error', __('Message does not empty!'));
        }
        $dt['message'] = $request->message;
        Mail::to($request->email)->send(new SendMail($dt));
        return redirect()->back()->with('toast_success', __('Message Sent Successfully!'));
