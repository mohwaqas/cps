...
class SizeController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($data) {
                    ...
                    $btn = $btn.'<a href="' . route('admin.product.size.edit', $data->id) . '" class="btn-action"><i class="fas fa-pencil-alt"></i></a>';
    ...
    public function productSizeStore(ProductSizeRequest $request){
        ...
        if ($size) {
            return redirect()->route('admin.product.size')->with('toast_success', __('Successfully Stored !'));
        }
    ...
    public function productSizeUpdate(Request $request){
        ...
        if ($color) {
            return redirect()->route('admin.product.size')->with('toast_success', __('Successfully Update !'));
        }
    ...
    public function productSizeDelete($id){
        ...
        if ($delete) {
            $delete->delete();
            return redirect()->route('admin.product.size')->with('toast_success', __('Successfully Deleted !'));
        }
        return redirect()->route('admin.product.size')->with('toast_error', __('Does Not Delete!'));
    }
}
