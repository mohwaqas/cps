...
class ItemTagController extends Controller
{
    ...
    public function itemTagStore(Request $request){
        ...
        if (!empty($tag)){
            return redirect()->route('admin.item.tag')->with('toast_success', __('Successfully stored'));
        }
    ...
    public function itemTagDelete($id){
        ...
        if ($delete) {
            $delete->delete();
            return redirect()->route('admin.item.tag')->with('toast_success', __('Successfully Deleted !'));
        }
        return redirect()->route('admin.item.tag')->with('toast_error', __('Does Not Delete!'));
    }
