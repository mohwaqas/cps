...
class ProductTagController extends Controller
{
    ...
    public function productTagStore(Request $request){
        if(is_null($request->tag)) {
            return redirect()->back()->with('toast_error', __('Tag field required!'));
        }
        $tag = ProductTagList::create([
           'name' => $request->tag
        ]);
        if (!empty($tag)){
            return redirect()->route('admin.product.tag')->with('toast_success', __('Successfully stored'));
        }
        return redirect()->back()->with('toast_error', __('Something is wrong'));
    }
    public function productTagDelete($id){
        ...
        if ($delete) {
            $delete->delete();
            return redirect()->route('admin.product.tag')->with('toast_success', __('Successfully Deleted !'));
        }
        return redirect()->route('admin.product.tag')->with('toast_error', __('Does Not Delete!'));
    }
