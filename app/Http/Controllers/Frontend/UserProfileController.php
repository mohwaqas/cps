...
class UserProfileController extends Controller
{
    ...
    public function myOrder(){
        ...
        if($user) {
            $data['all_orders'] = $orders->where('User_Id', $user->id)->whereIn('Order_Status', [ORDER_PENDING, ORDER_PROCESSING, ORDER_SHIPPED]);
        } else {
            $data['all_orders'] = [];
        }
        $data['delivered_orders'] = $orders->where('User_Id', $user->id)->where('Order_Status', ORDER_DELIVERED);
        $data['canceled_orders'] = $orders->where('User_Id', $user->id)->whereIn('Order_Status', [ORDER_CANCELLED, ORDER_DELIVERED_FAILED, ORDER_RETURN]);
