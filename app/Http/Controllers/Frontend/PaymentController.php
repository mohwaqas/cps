...
//    }
    ...
    public function orderCreateCall($order_number, $shipping_charge, $tax, $subtotal, $discount, $grand_total, $payment_method)
    {
        ...
        if($order['success'] == true) {
            ...
            \Cart::destroy();
            return redirect()->route('front')->with('toast_success','Payment With COD');
        }
    ...
    public function orederCreate($order_number, $shipping_charge, $tax, $subtotal, $discount, $grand_total, $payment_method, $payment_status)
    {
        ...
        $order=Order::create([
            ...
            'User_Id'=>Auth::id(),
            'package_id' => Auth::check() ? auth()->user()->package_id : 0,
    ...
    public function cancelled(Request $request)
    {
        if ($request->isMethod('POST')) {
            addActivity(__('Payment Cancelled!'), __('Payment cancelled by '). Auth::user()->name);
            return redirect()->route('user::package.index')->with('toast_error', 'Payment cancelled!');
