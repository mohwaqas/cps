    ...
    <div class="offcanvas-body">
                    ...
                    <div class="single-grid-product">
                        ...
                        <div class="product-info">
                            <div class="product-name-part">
                                ...
                                <button class="cart-remove-btn deleteItem" data-id="{{$item->rowId}}">{{__('Remove')}}</button>
                            </div>
                            <div class="product-price">
                                @if($user && (currencyConverter($item->discount_percent) > 0))
                                    <span class="regular-price">{{currencySymbol()[currency()]}} {{currencyConverter($item->weight *  $item->qty)}}</span>
                                @endif

                                @if($user)
                                    <span class="price">{{currencySymbol()[currency()]}} {{currencyConverter($item->price *  $item->qty)}}</span>
                                @endif
                            </div>
                        </div>
                    </div>
                ...
                <div class="empty-wish-list section">
                    <div class="container">
                        <div class="empty-box-wrap text-center">
                            ...
                            <p class="empty-box-content">{{__('Cart is empty. Please go to your home page for listing it.')}} </p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="total-bottom-part">
            <div class="total-count d-flex">
                <h3>{{__('Total')}}</h3>
                <h4 class="totalAmount">{{currencySymbol()[currency()]}} {{currencyConverter($total)}}</h4>
            </div>
