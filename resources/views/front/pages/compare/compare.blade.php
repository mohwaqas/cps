                                    ...
                                    @foreach($compareList as $item)
                                        <td class="product-image-title">
                                            <div class="product-top">
                                                @if($user)
                                                    <a href="{{route('single.product',$item->en_Product_Slug)}}" class="image"><img src="{{asset(ProductImage().$item->Primary_Image)}}" alt="Compare Product">
                                                    </a>
                                                @else
                                                    <img src="{{asset(ProductImage().$item->Primary_Image)}}" alt="Compare Product">
                                                @endif
                                            </div>
                                            <div>
                                                <h5>
                                                @if($user)
                                                    <a href="{{route('single.product',$item->en_Product_Slug)}}" class="title">{{langConverter($item->en_Product_Name,$item->fr_Product_Name)}}</a></h5>
                                                @else
                                                    {{langConverter($item->en_Product_Name,$item->fr_Product_Name)}}
                                                @endif
                                            </div>
                                        </td>
                                    @endforeach
                                ...
                                <tr>
                                    ...
                                    @foreach($compareList as $item)
                                        <td class="pro-addtocart">
                                            <a href="{{route('single.product',$item->en_Product_Slug)}}" class="primary-btn"><span>{{__('Add To Cart')}}</span>
                                            </a>
                                        </td>
                                    @endforeach
                                </tr>
