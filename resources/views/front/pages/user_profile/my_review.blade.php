                            ...
                            @forelse ($reviews as $review)
                                <div class="single-review-item bg-white d-flex align-items-center">
                                    <div class="review-left flex-shrink-0">
                                        ...
                                        @else
                                            <img class="product-thumbnail" src="{{asset(ProductImage().$review->product->Primary_Image)}}" alt="product">
                                        @endif
                                    </div>
                                    <div class="review-right flex-grow-1">
                                        <h4 class="product-name">
                                            @if($user)
                                                <a href="{{route('single.product',$review->product->en_Product_Slug)}}">{{$review->product->en_Product_Name}}</a>
                                            @else
                                                {{$review->product->en_Product_Name}}
                                            @endif
                                        </h4>
