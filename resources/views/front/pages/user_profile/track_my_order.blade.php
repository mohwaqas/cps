...
@section('content')
                                                ...
                                                <td>
                                                    <div class="item-image-lsit d-flex align-items-center">
                                                        <div class="single-item">
                                                            <img class="order-image" src="{{asset(ProductImage().$od->product->Primary_Image)}}" alt="images">
                                                        </div>
                                                    </div>
                                                </td>
                                                @if ($order->Order_Status == ORDER_DELIVERED)
                                                    ...
                                                    @else
                                                        <td><button class="primary-btn write-review-btn" data-bs-toggle="modal" data-bs-target="#writeReviewModal{{$od->id}}">
                                                                {{__('Review')}}</button></td>
                                                    @endif
                                                @endif
                                                <td>
                                                    <span class="amount">{{currencySymbol()[currency()].currencyConverter($od->Price)}}</span>
                                                </td>
                                                <td>{{currencyConverter($od->Quantity)}}</td>
                                                <td>{{currencySymbol()[currency()].currencyConverter($od->Total_Price)}}</td>
                                            </tr>
                                        @endforeach
                                        <tr>
                                            <td></td>
                                            ...
                                            <td></td>
                                            <td>{{__('Subtotal')}}</td>
                                            <td>{{currencySymbol()[currency()].currencyConverter($order->Sub_Total)}}</td>
                                        </tr>
                                        <tr>
                                            <td></td>
                                            <td></td>
                                            ...
                                            <td></td>
                                            <td>{{__('Tax')}}</td>
                                            <td>{{currencySymbol()[currency()].currencyConverter($order->Tax)}}</td>
                                        </tr>
                                        <tr>
                                            <td></td>
                                            <td></td>
                                            ...
                                            <td>{{currencySymbol()[currency()].currencyConverter($order->Delivery_Charge)}}</td>
                                        </tr>
                                        <tr>
                                            <td></td>
                                            <td></td>
                                            ...
                                            <td></td>
                                            <td>{{__('Discount')}}</td>
                                            <td>{{currencySymbol()[currency()].currencyConverter($order->admin_discount)}}</td>
                                        </tr>
                                        <tr>
                                            <td></td>
                                            <td></td>
    ...
    @if ($order->Order_Status == ORDER_DELIVERED)
        @foreach ($order->order_details as $domd)
            <div class="modal fade writeReviewModal" id="writeReviewModal{{$domd->id}}" tabindex="-1" aria-labelledby="writeReviewModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content p-5">
                        <div class="modal-header">
                            <h2 class="modal-title fw-bold text-black" id="writeReviewModalLabel">{{__('Write Your Feedback')}}</h2>
                        ...
                        <div class="modal-body">
                            <form id="feedback" method="POST" action="{{route('user.profile.review_store')}}">
                                ...
                                <div class="mb-3  w-100">
                                    <label for="exampleFormControlTextarea1" class="form-label">{{__('Rating')}}</label><br>
                                ...
                                <div class="mb-3">
                                    <label for="exampleFormControlTextarea1" class="form-label">{{__('Write Your Feedback')}}</label>
